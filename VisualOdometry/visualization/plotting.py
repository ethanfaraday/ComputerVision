...
def visualize_paths(gt_path, pred_path, html_tile="", title="VO exercises", file_out="plot.html"):
    ...
    fig1 = figure(title="Paths", tools=tools, match_aspect=True, width_policy="max",
                toolbar_location="above", x_axis_label="x", y_axis_label="y")

    # Ground truth (points + line)
    fig1.circle("gtx", "gty", source=source, color="blue", size=4,
    ...
    fig1.circle("px", "py", source=source, color="green", size=4,
                hover_fill_color="firebrick", legend_label="Pred")
    fig1.line("px", "py", source=source, color="green", legend_label="Pred")

    # Displacement/error vectors (ensure disx/disy are list-of-lists for multi_line)
    fig1.multi_line("disx", "disy", source=source, legend_label="Error",
                    color="red", line_dash="dashed")

    fig1.legend.click_policy = "hide"

    fig2 = figure(title="Error", tools=tools, width_policy="max",
                toolbar_location="above", x_axis_label="frame", y_axis_label="error")

    fig2.circle("diffx", "diffy", source=source, size=4,
    ...
    show(layout([
        ...
        Div(text="<h2>Paths</h2>"),     # fixed closing tag
        [fig1, fig2],
